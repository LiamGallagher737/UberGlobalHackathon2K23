...
export const POST: RequestHandler = async ({ params, locals }) => {
    ...
    if (my_user.friends && my_user.friends.includes(other_user.id))
        return json({ status: 'already friends' } satisfies AddFriendResponse, { status: 208 });

    const meToYou = conn.execute(
        sql`UPDATE "user" SET friends = ARRAY_APPEND(friends, ${other_user.id}) WHERE id = ${my_user.id};`
    );

    const youToMe = conn.execute(
        sql`UPDATE "user" SET friends = ARRAY_APPEND(friends, ${my_user.id}) WHERE id = ${other_user.id};`
    );

    await Promise.all([meToYou, youToMe]);

    return json({ status: 'ok' } satisfies AddFriendResponse);
};
