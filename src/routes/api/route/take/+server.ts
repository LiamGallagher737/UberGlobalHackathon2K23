...
export const POST: RequestHandler = async ({ locals, request }) => {
    ...
    if (!data.journey_ID)
        throw error(422, "No journey_ID provided");

    const journey = (await conn
        .select()
        .from(journeys)
        .where(eq(journeys.id, data.journey_ID))
        .limit(1))[0];

    if (!journey.points || !journey.ownerEmail)
        throw error(404, "Unable to find journey");


    const currentPoints = (await conn
        .select({ points: users.points })
        .from(users)
        .where(eq(users.email, email)))[0];

    if (!currentPoints.points)
        throw error(500, "Unable to fetch user");
